...
john.yft.diag <- read.MFCLRep("c:/x/yft/john_yft_diag/plot-14.par.rep")
...
b <- flr2taf(SB(penguin.diagnostic.10)/1e6, colname="SB")
b0 <- flr2taf(SBF0(penguin.diagnostic.10)/1e6, colname="SBF0")
pdiag10 <- merge(b, b0)
pdiag10$Depletion <- pdiag10$SB / pdiag10$SBF0

b <- flr2taf(SB(penguin.diagnostic.12)/1e6, colname="SB")
b0 <- flr2taf(SBF0(penguin.diagnostic.12)/1e6, colname="SBF0")
pdiag12 <- merge(b, b0)
